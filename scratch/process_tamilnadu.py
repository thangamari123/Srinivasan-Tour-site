...
src_dir = r'public/images/temp-tamilnadu'
target_dir = r'public/images/tamilnadu-tour-6-days'
...
for f in files:
    if os.path.isfile(f):
        ...
        new_images.append(f'/images/tamilnadu-tour-6-days/{sanitized}')

# Cleanup temp dir
shutil.rmtree(src_dir, ignore_errors=True)

print(f'Processed {len(new_images)} images for Tamil Nadu tour.')
    ...
    if img_start != -1:
        ...
        if img_end != -1:
            ...
            with open(pkg_file, 'w', encoding='utf-8') as f:
                f.write(content)
            print('Successfully updated packages.ts for tamilnadu-tour-6-days!')
        else:
            print('Error: Could not find end of images array')
    else:
        print('Error: Could not find images array start')
else:
    print('Error: Could not find tamilnadu-tour-6-days package')
